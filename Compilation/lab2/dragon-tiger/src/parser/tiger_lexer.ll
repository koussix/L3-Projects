...
{integer} {
          if(strlen(yytext)>=2 && yytext[0]=='0'){
            utils::error(loc,"Le format de l'entier est eronné");
            exit(1);
          }

          long int valeur = strtol(yytext, NULL ,10);
          if(valeur <= TIGER_INT_MAX)
            return yy::tiger_parser::make_INT(strtol(yytext, NULL ,10), loc);
          else{
            utils::error(loc,"La valeur de l'entier est superieur à la valeur max"); 
            exit(1);
            }
    }
